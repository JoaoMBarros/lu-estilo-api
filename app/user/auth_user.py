...
class UserCases:
    ...
    def user_register(self, user: UserBase):
        ...
        # Using the UserCreate schema to validate the data
        user = UserCreate(name=user.name, email=user.email, password=hashed_password, refresh_token=refresh_token)
    ...
    def authenticate_user(self, user_data: UserLogin):
        ...
        user = UserCreate(name=db_user.name, email=db_user.email, password=db_user.password, refresh_token=db_user.refresh_token)
